...
export const subscribeToChannel = async (req, res) => {
    ...
    try {
        ...
        const currUser = await User.findByIdAndUpdate({ _id: req.user.id }, { $addToSet: { subscribedChannels: channel._id } }, { new: true })
        res.status(200).json(currUser)
    } catch (error) {
        console.log(error.message);
        res.status(400).json(error.message)
...
export const incrementSubscribersCount = async (req, res) => {
    if (req.user.id === req.params.userId) return res.status(400).json('You are not allowed to subscribe to yourself')
    try {
        const user = await User.findOneAndUpdate({_id: req.params.userId}, {$inc: {subscribers: 1}}, {new: true})
...
export const unSubscribeToChannel = async (req, res) => {
    ...
    try {
        const channel = await User.findById(req.params.userId)
        if (!channel) return res.status(400).json('Channel does not exist')
        const currUser = await User.findByIdAndUpdate({ _id: req.user.id }, { $pull: { subscribedChannels: channel._id } }, { new: true })
        res.status(200).json(currUser)
    } catch (error) {
        console.log(error.message);
        res.status(400).json(error.message)
